...
- [x] Frontend: `index.html` SSR marker `<!--worldcup2-ssr-og-->`; branded `public/worldcup2-og-card.svg`
...
- Crawlers do not execute client JS; meta tags must appear in the first HTML document. The API serves that document for `/league/:address` by reading the SPA `index.html` template and replacing `<!--worldcup2-ssr-og-->` with escaped meta tags.
...
- Default `og:image` is `${PUBLIC_APP_ORIGIN}/worldcup2-og-card.svg` (1200×630 branded shell). Some networks prefer PNG; swap asset later if needed.
...
- `backend/.env.example`
- `frontend/index.html`
- `frontend/public/worldcup2-og-card.svg`
