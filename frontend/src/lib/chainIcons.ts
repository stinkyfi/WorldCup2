/**
 * Navbar chain icons — same artwork as @rainbow-me/rainbowkit chainIcons (inline SVG data URLs).
 * wagmi `useChains()` does not include RainbowKit-merged `iconUrl`, so we map by chain id here.
 */
const RK_BASE_ICON =
...
const FALLBACK_ICON_BY_ID: Record<number, string> = {
  1: RK_ETHEREUM_ICON,
  8453: RK_BASE_ICON,
  84532: RK_BASE_ICON,
};

const SONIC_ICON = "/chains/sonic.png";

export function getChainIconSrc(chain: {
  id: number;
  iconUrl?: string | (() => Promise<string>) | null;
}): string | undefined {
  const { iconUrl, id } = chain;
  if (typeof iconUrl === "string" && iconUrl.length > 0) return iconUrl;
  if (id === 146) return SONIC_ICON;
  return FALLBACK_ICON_BY_ID[id];
}
