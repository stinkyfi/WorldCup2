...
const prefix = "__merkle_fee_claim_test__";
...
test(
  "GET /api/v1/merkle-claim/fee returns proof when row exists",
  { skip: !RUN },
  async () => {
    const leagueAddr = "0xeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeee";
    ...
    const res = await app.inject({
      method: "GET",
      url: `/api/v1/merkle-claim/fee?chainId=84532&leagueAddress=${leagueAddr}&walletAddress=${wallet}`,
    });
    ...
    assert.equal(body.data.amountWei, "123000");
...
test(
  "GET /api/v1/merkle-claim/fee returns eligible false without row",
  { skip: !RUN },
  async () => {
    const leagueAddr = "0xffffffffffffffffffffffffffffffffffffffff";
    await prisma.league.create({
      data: {
        chainId: 84532,
        title: `${prefix} other`,
    ...
    const res = await app.inject({
      method: "GET",
      url: `/api/v1/merkle-claim/fee?chainId=84532&leagueAddress=${leagueAddr}&walletAddress=0xdddddddddddddddddddddddddddddddddddddddd`,
    });
    assert.equal(res.statusCode, 200);
    const body = JSON.parse(res.body) as { data: { eligible: boolean; reason?: string } };
    assert.equal(body.data.eligible, false);
    assert.equal(body.data.reason, "NO_FEE_LEAF");
    await app.close();
  },
);
